...
PURE TRAINING: an example supplies an objective ``(theta, batch) -> scalar loss``; the
Adam loop here is domain-agnostic. The losses read a ``LogVec[bool]`` sentence directly
...
def neg_log(sat: LogVec[bool]) -> torch.Tensor:
    """Knowledge loss ``J = mean -log(SAT)`` of a LogVec Bool sentence (the categorical
    NLL) — the standard knowledge loss."""
    return log_vec_nll(sat).mean()


def cross_entropy(pred_sat: LogVec[bool], label_sat: LogVec[bool]) -> torch.Tensor:
...
def train_batched[D, B, M: torch.nn.Module](
    ...
    """Minimize ``objective`` over θ with Adam. θ is any ``torch.nn.Module`` (an arbitrary
    network); it is updated in place (the optimizer steps the module's own parameters) and
    returned."""
    theta = theta0
    opt = torch.optim.Adam(theta.parameters(), lr=learning_rate, betas=(0.9, 0.999))

    print_every = max(1, num_epochs // 20)
    last_loss = 0.0
    for epoch in range(num_epochs):
        for batch in mk_batches(epoch, data):
            loss = objective(theta, batch)
            opt.zero_grad()
